...
import { isAccountUpgraded, upgradeToSmartAccount } from '../services/tokenDelegationService';
...
const WalletConnection = () => {
  ...
  // Función para verificar si la cuenta es una Smart Account
  const checkSmartAccountStatus = async () => {
    if (!isConnected || !account) return;
    
    setCheckingUpgrade(true);
    try {
      const result = await isAccountUpgraded(account);
      setIsSmartAccount(result);
    } catch (err) {
  ...
  // Función para actualizar la cuenta a Smart Account
  const handleUpgradeToSmartAccount = async () => {
    if (!isConnected || !account) return;

    setCheckingUpgrade(true);
    try {
      const result = await upgradeToSmartAccount(account);
      if (result.success) {
        // Actualizar el estado para reflejar que ahora es una Smart Account
    ...
    <div className="w-full max-w-md mx-auto p-6 bg-white dark:bg-gray-800 rounded-xl shadow-lg">
          ...
          <div className="text-center">
            ...
            {isConnected && (
              <div className="mt-2">
                ...
                <button
                  onClick={checkSmartAccountStatus}
                ...
                >
                  {checkingUpgrade ? 'Checking...' : 'Check Status'}
                </button>
              </div>
            )}
          </div>
        ...
        <div className="flex flex-col space-y-3 mt-6">
          {!isConnected ? (
            ...
            <>
              {!isSmartAccount && (
                <button
                  onClick={handleUpgradeToSmartAccount}
                  disabled={checkingUpgrade}
      ...
      <div className="mt-8 text-sm text-gray-600 dark:text-gray-400">
        ...
        <ul className="list-disc pl-5 space-y-1">
          ...
          <li>Smart Accounts required for advanced permissions features</li>
        </ul>
      </div>
    </div>
  );
